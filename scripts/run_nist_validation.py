...
NIST_REF_DIR = ROOT / "tests" / "data" / "nist_reference"
...
class PartitionFunctionCheck:
    ...
    @staticmethod
    def run(solver):
        nist_ref = _load_nist_json("partition_functions.json")
        results = {}
        for elem, stages in nist_ref.items():
            results[elem] = {}
            for stage_str, temps in stages.items():
                stage_res = {}
                for T_K_str, nist_U in temps.items():
                    T_K = int(T_K_str)
                    T_eV = T_K / EV_TO_K
                    our_U = solver.calculate_partition_function(elem, int(stage_str), T_eV)
                    pct = (our_U - nist_U) / nist_U * 100
                    stage_res[T_K_str] = {"cflibs": round(our_U, 3), "nist": nist_U, "pct_diff": round(pct, 2)}
                results[elem][stage_str] = stage_res
        return results
...
class IonizationFractionCheck:
    ...
    @staticmethod
    def run(solver):
        nist_ref = _load_nist_json("ionization_fractions.json")
        results = {}
        for elem, ref in nist_ref.items():
            fracs = solver.get_ionization_fractions(elem, ref["T_eV"], ref["n_e"])
            elem_res = {}
            for stage in sorted(set(list(fracs.keys()) + [int(s) for s in ref["fractions"]])):
                our = fracs.get(stage, 0.0)
                nist = ref["fractions"].get(str(stage))
                if nist is not None:
...
def main():
    ...
    print(f"Ionization fractions: {ion_pass}/{ion_total} within 10% of NIST")
