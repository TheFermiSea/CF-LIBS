...
class ALIASIdentifier:
    ...
    def _detect_peaks(
        ...
        if not np.isfinite(wl_step) or wl_step <= 0:
            wl_step = 1.0  # safe fallback
        if self.resolving_power and self.resolving_power > 0 and np.median(wavelength) > 0:
            resolution_nm = float(np.median(wavelength)) / self.resolving_power
        else:
            resolution_nm = 0.1
        min_distance_px = max(1, int(resolution_nm / max(wl_step, 1e-9)))
        # ALIAS uses strong prominence (=threshold) because it relies on
        # spectral fingerprint correlation, not individual peak quality.
