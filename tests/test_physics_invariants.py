...
def _make_spectrum_model(atomic_db, T_K, n_e, element="Fe", **kw):
    ...
    return SpectrumModel(
        plasma=plasma, atomic_db=atomic_db, instrument=instrument,
...
class TestNonNegativeIntensity:
    ...
    @pytest.mark.parametrize("T_K, n_e", [
        (8000.0, 1e16), (10000.0, 1e17), (15000.0, 1e17), (20000.0, 1e18),
...
class TestPathLengthScaling:
    ...
    def test_double_path_doubles_intensity(self, atomic_db):
        ...
        common = dict(
            plasma=plasma, atomic_db=atomic_db, instrument=instrument,
            lambda_min=370.0, lambda_max=376.0, delta_lambda=0.01,
        )
        ...
        np.testing.assert_allclose(I2, 2.0 * I1, rtol=1e-10,
...
class TestIonizationBalance:
    ...
    def test_higher_ne_suppresses_ionization(self, solver):
        ...
        assert f_hi.get(1, 0) >= f_lo.get(1, 0), (
